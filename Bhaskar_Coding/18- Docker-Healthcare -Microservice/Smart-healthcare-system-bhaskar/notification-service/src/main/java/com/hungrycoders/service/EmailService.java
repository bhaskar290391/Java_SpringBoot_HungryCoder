...
public class EmailService {
	...
	public void triggerEmailNotification(Appointment appointment) {
		...
		case PENDING:
			...
			text = String.format(APPOINTMENT_PENDING_EMAIL_BODY, patient,
					appointment.getAppointmentTime().format(dateTimeFormatter), appointmentNotes, doctorComments);
			break;

		case CONFIRMED:
			to = appointment.getPatient().getEmail();
			subject = APPOINTMENT_CONFIRMED_SUBJECT;
			text = String.format(APPOINTMENT_CONFIRMED_EMAIL_BODY, doctor,
					appointment.getAppointmentTime().format(dateTimeFormatter), appointmentNotes, doctorComments);
			break;
		...
		case REJECTED:
			...
			text = String.format(APPOINTMENT_REJECTED_EMAIL_BODY, doctor, appointmentNotes, doctorComments);
			break;

		case COMPLETED:
			to = appointment.getPatient().getEmail();
			subject = APPOINTMENT_COMPLETED_SUBJECT;
			text = String.format(APPOINTMENT_COMPLETED_EMAIL_BODY, doctor,
					appointment.getAppointmentTime().format(dateTimeFormatter), appointmentNotes, doctorComments);
			break;
		...
		default:
			...
			text = String.format(APPOINTMENT_UPDATED_EMAIL_BODY, doctor,
					appointment.getAppointmentTime().format(dateTimeFormatter), appointmentNotes, doctorComments);
		}
